...
walkMaker <- function(trkdata,trkinfo,walkIDs,startIDs=walkIDs[1:2],
  ...
  if (min_dist_pts %in% c("beg2end","beg2beg")) seg1 <- seg1[nrow(seg1):1,]
  if (min_dist_pts %in% c("beg2end","end2end")) seg2 <- seg2[nrow(seg2):1,]
  walkdat <- rbind(seg1,seg2)
  
  ## Loop through other tracks and append
  if (length(walkIDs)>2) {
    for (i in 3:length(walkIDs)) {
      nextseg <- dplyr::filter(trkdata,.data$ID==walkIDs[i]) %>%
      ...
      if (d_end2end<d_end2beg) nextseg <- nextseg[nrow(nextseg):1,]
...
iOrderWalk <- function(walkIDs,trkinfo,startIDs=walkIDs[1:2]) {
  ...
  repeat {
    ...
    tmp <- dplyr::filter(trkinfo,.data$ID==walk[length(walk)])
